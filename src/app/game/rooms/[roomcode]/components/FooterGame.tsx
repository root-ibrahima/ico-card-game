...
const FooterGame: React.FC<FooterGameProps> = ({
  ...
  const footerColor =
    role.toLowerCase() === 'pirate'
      ? '#EF4B4B'
      : role.toLowerCase() === 'marin'
      ? '#3B60BC'
      : role.toLowerCase() === 'sirene'
      ? '#46CC4F'
  ...
  return (
    ...
    >
      ...
      <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 bg-white w-16 h-16 rounded-lg shadow-md flex items-center justify-center">
        <Image
          src={`/cartes/roles/Carte-${role.toLowerCase()}.png`} // Assurez-vous que le rôle est en minuscule
          alt={role}
          width={64}
          height={64}
